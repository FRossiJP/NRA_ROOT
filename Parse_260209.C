...
/*
	...
	How to run this macro:
		Single file -> root -b -q Parse_260209.C\(\"list/list_[Run#].asc\",Ql_MIN,Ql_MAX\)
		All files -> for i in list/list_*.asc; do root -b -q Parse_260209.C\(\"$i\",Ql_MIN,Ql_MAX\); done
				  -> for i in list/list_*.asc; do root -b -q Parse_260209.C\(\"$i\",100,60000\); done
				  -> for i in list/list_*.asc; do root -b -q Parse_260209.C\(\"$i\",500,60000\); done
				  -> for i in list/list_*.asc; do root -b -q Parse_260209.C\(\"$i\",1000,60000\); done
				  -> for i in list/list_*.asc; do root -b -q Parse_260209.C\(\"$i\",1500,60000\); done
				  -> for i in list/list_*.asc; do root -b -q Parse_260209.C\(\"$i\",2000,60000\); done
...
void Parse_260209(string file, Double_t Ql_MIN, Double_t Ql_MAX)	//main function like (cint)
{
//	const TString MainDir = string(fs::current_path());
	...
	if (PrintMemoON == 1)
	{
		...
		cout << "    (exp. 2025): mGS20 / H01   / H02   / H03   / H04   / H05   / H06   / H07   / H08   / H09   / H10   / EJ270 / EJ270 /       /       / Linac /" << endl << endl;
